  ...
  public static syncAutomaticNotifications(): void {
    ...
    debts.forEach(debt => {
      ...
      const amtDisplay = isPartial
        ? `salio la Sh ${remainingStr} (Jumla: Sh ${debt.amount.toLocaleString()})`
        : `Sh ${remainingStr}`;

      const shopName = debt.shopId
        ? shops.find(s => s.id === debt.shopId)?.name || 'Duka'
        : undefined;
  ...
  public static notifySaleEditReviewed(
    ...
    const notification: AppNotification = {
      ...
      message: isApproved
        ...
        : `Ombi lako la kuhariri mauzo limekataliwa na ${adminName}.${
            reviewNote ? ` Sababu: ${reviewNote}` : ''
          }`,
      isGlobal: false,
